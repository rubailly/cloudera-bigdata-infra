...
echo "3. Testing SSH key setup..."
echo -n "  - Testing passwordless SSH: "
for server in $MASTER_IP $WORKER1_IP $WORKER2_IP; do
  ssh -o BatchMode=yes $SSH_USER@$server "echo OK" > /dev/null 2>&1 || { echo "FAILED - SSH keys not set up correctly for $server"; exit 1; }
done
echo "OK"
